...
initial <- read.csv(list.files(pattern = "ALL-B12_initial_"), as.is = T, na.strings = c(""))
...
initial_ct <- CutField(initial)
colnames(initial_ct) <- paste0("i_", colnames(initial_ct))
...
m3 <- merge(m2, initial_ct, by.x = "r_症例登録番号", by.y = "i_症例登録番号", all.x = T)

# ALL-B12 基本的な情報をマージするプログラム Mamiko Yonejima 20170817

# 関数の設定 列抽出とAEgrade切り離し、ファイルの出力をする関数
MakeDataSet <- function(dataframe) {
  if (flg == 1) {
    datecut_df <- dataframe[format(as.Date(dataframe$作成日), "%Y%m%d") <= kDateShimekiri, c(2, 4:6, 8, 11, 13, 15, 18, 29:length(colnames(dataframe)))]
    flowsheet_df <- datecut_df[, c(1:9, seq(9, length(colnames(datecut_df)), by = 2))]
  } else {
    datecut_df <- dataframe[format(as.Date(dataframe$作成日), "%Y%m%d") <= kDateShimekiri, c(2, 4:6, 8, 11, 13, 15, 18, 29:length(colnames(dataframe)))]
    flowsheet_df <- datecut_df[format(as.Date(datecut_df$作成日), "%Y%m%d") >= kDateShimekiri_start, c(1:9, seq(9, length(colnames(datecut_df)), 
                                                                                                             by = 2))]
  }
...
MakeDataSet_1 <- function(dataframe) {
  if (flg == 1) {
    ...
    datecut_df[format(as.Date(datecut_df$作成日), "%Y%m%d") >= kDateShimekiri_start, c(2, 4:6, 8, 11, 13, 15, 18, seq(28, length(colnames(datecut_df)), 
                                                                                                                   by = 2))]
  } else {
    datecut_df <- dataframe[format(as.Date(dataframe$作成日), "%Y%m%d") <= kDateShimekiri, ]
    datecut_df[is.na(datecut_df)] <- ""
...
merge_3 <- merge(merge_2, cutdate_cancel, by = "症例登録番号", all = T)
base_df <- merge(merge_3, cutdate_cancel2, by = "症例登録番号", all = T)
# flowsheet作成
for (i in c(1, 3:42)) {
...
write.csv(flowsheet_df, "risk1.csv")
flowsheet_df <- MakeDataSet_1(risk2)
write.csv(flowsheet_df, "risk2.csv")
flowsheet_df <- MakeDataSet_1(risk3)
write.csv(flowsheet_df, "risk3.csv")
flowsheet_df <- MakeDataSet_1(cancel)
write.csv(flowsheet_df, "cancel.csv")
flowsheet_df <- MakeDataSet_1(cancel2)
write.csv(flowsheet_df, "cancel2.csv")
